...
import { createSession, createIntent, getPayment, listPayments, listPaymentEvents } from './api'
...
export default function App() {
  ...
  const [intentForm, setIntentForm] = useState({ provider:'auto', amount_cents:50000, currency:'usd', order_id:'ORD-002', capture_mode:'automatic' })
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      if (paymentId) setResult(await getPayment(paymentId))
      await refreshTables()
    }, refreshMs)
  ...
  return (
    <div className="app">
      <div className="card">
        ...
        <button className="btn" onClick={async () => { setResult(await createSession(sessionForm)); await refreshTables() }}>Create session</button>
      </div>
      ...
      <div className="card">
        ...
        <button className="btn" onClick={async () => { setResult(await createIntent(intentForm)); await refreshTables() }}>Create intent</button>
      </div>
      ...
      <div className="card">
        ...
        <table className="table">
          <thead><tr><th>ID</th><th>Provider</th><th>Event</th><th>Status</th><th>Created</th></tr></thead>
          <tbody>
            {events.map(e => <tr key={e.id}><td>{e.id}</td><td>{e.provider}</td><td>{e.event_type}</td><td>{e.normalized_status}</td><td>{e.created_at}</td></tr>)}
          </tbody>
        </table>
      </div>
