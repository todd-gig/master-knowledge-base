...
import { createSession, createIntent, getPayment, getTenantPolicy, putTenantPolicy } from './api'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => setResult(await getPayment(paymentId)), refreshMs)
  ...
  return (
    <div className="app">
      ...
      <div className="card">
        <h2>Tenant payment policy</h2>
        <textarea value={policyText} onChange={e => setPolicyText(e.target.value)} rows={12} />
        <button className="btn" onClick={async () => setPolicy(await putTenantPolicy(JSON.parse(policyText)))}>Save policy</button>
      </div>
